...
def get_article_parallel(begin_page,number_page,research,search_term,api_query='http://paperity.org/api/1.0/search/'):
    ...
    for i in range(begin_page, begin_page + number_page):
        ...
        for paper in json_data['papers']:
            ...
            if article.exists():
                ...
                fulltext = pdf.extract_full_text(metadata['url'],"page_"+str(begin_page)+"_research_"+str(research.id))
                title = metadata['title']
                abstract = metadata['abstract']
                url_file = metadata['url']
                article = Article.objects.create(title=title,doi=doi,abstract=abstract,fulltext=fulltext,publication=publication,url_file=url_file)
            
            #we associate this article to the research
...
def Get_ID(search_term,research,number_threads = 1):
    ...
    for thread in range(number_threads):
        begin = current_page
        Thread_id.append(Thread(target=get_id_parallel,args=(begin,number_page_by_thread,research)))
        current_page += number_page_by_thread
    # we build the last thread with the last pages
    begin = current_page
    last_pages = max_page - current_page
    Thread_id.append(Thread(target=get_id_parallel,args=(begin,last_pages,research)))
...
def Get_PAP(search_term, name):
    
    # we prepare the research text with parenthesis, quotes and logical operations
    split_terme = split_search_term(search_term)
    search = ""
    size = len(split_terme)
    for i in range(size):
        search += split_terme[i]
        search += "+"
